...
IMU::IMU(ros::NodeHandle &nh,std::string port, unsigned int baudrate) : _nh(nh),iMyAhrsPlus(port, baudrate) {
  _pub = _nh.advertise<sensor_msgs::Imu>("/life/imu",1);
}
...
void IMU::operate_data(int sensor_id) {
  ...
  _imu_msg.orientation.x = sensor_data.quaternion.x;
  _imu_msg.orientation.y = sensor_data.quaternion.y;
  _imu_msg.orientation.z = sensor_data.quaternion.z;
  _imu_msg.orientation.w = sensor_data.quaternion.w;
